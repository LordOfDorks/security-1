...
void DiceVolatileData(FILE* f)
{
    ...
    DicePrintData(f, "          DeviceRootName", diceVolatileData->info.diceRootNames[DICE_DEVICE_CLASS], sizeof(diceVolatileData->info.diceRootNames[DICE_DEVICE_CLASS]));
//    DicePrintData(f, "        DeviceCompoundId", diceVolatileData->info.diceCompoundIds[DICE_DEVICE_CLASS], sizeof(diceVolatileData->info.diceCompoundIds[DICE_DEVICE_CLASS]));
    DicePrintData(f, "      DeviceCompoundName", diceVolatileData->info.diceCompoundNames[DICE_DEVICE_CLASS], sizeof(diceVolatileData->info.diceCompoundNames[DICE_DEVICE_CLASS]));
#ifdef DICE_INCLUDE_MANUFACTURERID
    DicePrintData(f, "    ManufacturerRootName", diceVolatileData->info.diceRootNames[DICE_MANUFACTURER_CLASS], sizeof(diceVolatileData->info.diceRootNames[DICE_MANUFACTURER_CLASS]));
//    DicePrintData(f, "  ManufacturerCompoundId", diceVolatileData->info.diceCompoundIds[DICE_MANUFACTURER_CLASS], sizeof(diceVolatileData->info.diceCompoundIds[DICE_MANUFACTURER_CLASS]));
    DicePrintData(f, "ManufacturerCompoundName", diceVolatileData->info.diceCompoundNames[DICE_MANUFACTURER_CLASS], sizeof(diceVolatileData->info.diceCompoundNames[DICE_MANUFACTURER_CLASS]));
#endif
    DicePrintData(f, "           AppStrongName", diceAppPayloadInfo->strongName, sizeof(diceAppPayloadInfo->strongName));
#ifdef DICE_INCLUDE_OPERATORID
//    DicePrintData(f, "              OperatorId", diceVolatileData->info.diceCompoundIds[DICE_OPERATOR_CLASS], sizeof(diceVolatileData->info.diceCompoundIds[DICE_OPERATOR_CLASS]));
    DicePrintData(f, "            OperatorName", diceVolatileData->info.diceCompoundNames[DICE_OPERATOR_CLASS], sizeof(diceVolatileData->info.diceCompoundNames[DICE_OPERATOR_CLASS]));
#endif
    fprintf(f, "-------------------------------------------------------------------\r\n");
}
...
HAL_StatusTypeDef DiceCalculateCMAC(uint8_t encrypt, uint8_t* dataPtr, uint32_t dataSize, uint8_t* keyIn, uint8_t* digestIn, uint8_t* digestOut)
{
    ...
    else
    {
        ...
        ((uint32_t*)digestOut)[3] = __REV(hcryp.Instance->IVR0);
        ((uint32_t*)digestOut)[2] = __REV(hcryp.Instance->IVR1);
        ((uint32_t*)digestOut)[1] = __REV(hcryp.Instance->IVR2);
        ((uint32_t*)digestOut)[0] = __REV(hcryp.Instance->IVR3);
    }
...
HAL_StatusTypeDef DiceDeriveData(void)
{
    ...
    if(diceProtectedData->tag == DICE_PROTECTED_PERSISTED_DATA_TAG)
    {
        ...
        if((retVal = DiceCalculateCMAC(DICE_CMAC_ENCRYPT,
                                       ...
                                       NULL,
                                       diceVolatileData->info.diceRootNames[DICE_DEVICE_CLASS])) != HAL_OK)
        {
            goto Cleanup;
        }
        ...
        if((retVal = DiceCalculateCMAC(DICE_CMAC_ENCRYPT,
                                       ...
                                       NULL,
                                       diceVolatileData->info.diceRootNames[DICE_MANUFACTURER_CLASS])) != HAL_OK)
        {
            goto Cleanup;
        }
#endif
        ...
        if(diceAppPayloadInfo->tag == DICE_APPPAYLOAD_INFO_TAG)
        {
            ...
            if((retVal = DiceCalculateCMAC(DICE_CMAC_ENCRYPT,
                                           ...
                                           diceProtectedData->deviceID,
                                           NULL,
                                           diceVolatileData->info.diceCompoundIds[DICE_DEVICE_CLASS])) != HAL_OK)
            {
                goto Cleanup;
            }
            ...
            if((retVal = DiceCalculateCMAC(DICE_CMAC_ENCRYPT,
                                           ...
                                           diceProtectedData->manufacturerID,
                                           NULL,
                                           diceVolatileData->info.diceCompoundIds[DICE_MANUFACTURER_CLASS])) != HAL_OK)
            {
                goto Cleanup;
            }
#endif
            ...
            if((retVal = DiceCalculateCMAC(DICE_CMAC_DECRYPT,
                                           ...
                                           operatorIdKey,
                                           NULL,
                                           diceVolatileData->info.diceCompoundIds[DICE_OPERATOR_CLASS])) != HAL_OK)
            {
                goto Cleanup;
            }
#endif
            ...
            if((retVal = DiceCalculateCMAC(DICE_CMAC_ENCRYPT,
                                           ...
                                           diceVolatileData->info.diceCompoundIds[DICE_DEVICE_CLASS],
                                           NULL,
                                           diceVolatileData->info.diceCompoundNames[DICE_DEVICE_CLASS])) != HAL_OK)
            {
                goto Cleanup;
            }
            ...
            if((retVal = DiceCalculateCMAC(DICE_CMAC_ENCRYPT,
                                           ...
                                           NULL,
                                           diceVolatileData->info.diceCompoundNames[DICE_MANUFACTURER_CLASS])) != HAL_OK)
            {
                goto Cleanup;
            }
#endif
            ...
            if((retVal = DiceCalculateCMAC(DICE_CMAC_ENCRYPT,
                                           ...
                                           NULL,
                                           diceVolatileData->info.diceCompoundNames[DICE_OPERATOR_CLASS])) != HAL_OK)
            {
                goto Cleanup;
            }
#endif
